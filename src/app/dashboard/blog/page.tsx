    ...
    <div className="space-y-4">
      <Card title="Blog Brief" icon="📋">
        <dl className="grid sm:grid-cols-2 gap-4 text-sm mb-4">
          <div>
            <dt className="text-xs text-white mb-0.5">Title</dt>
            <dd className="text-zinc-200 font-medium">{brief.title ?? "—"}</dd>
          </div>
          <div>
            <dt className="text-xs text-white mb-0.5">Target word count</dt>
            <dd className="text-zinc-200 font-medium">{brief.target_word_count ?? "—"} words</dd>
          </div>
          <div className="sm:col-span-2">
            <dt className="text-xs text-white mb-0.5">Meta description</dt>
            <dd className="text-white">{brief.meta_description ?? "—"}</dd>
          </div>
        </dl>

        {(brief.h2_sections ?? []).length > 0 && (
          <div className="mb-4">
            <p className="text-xs font-semibold text-white uppercase tracking-wider mb-2">H2 Sections</p>
            <ol className="space-y-1">
              {brief.h2_sections.map((h: string, i: number) => (
                <li key={i} className="text-sm text-white flex gap-2">
                  <span className="text-white font-mono shrink-0">{i + 1}.</span>{h}
                </li>
              ))}
            </ol>
          </div>
      ...
      {data.rewritten_content && (
        <Card title={`Blog Post (${data.word_count ?? 0} words)`} icon="✍️">
          <div
            className={`text-sm text-white leading-relaxed overflow-hidden transition-all ${showFull ? "" : "max-h-64"}`}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-white mt-1">Generate a fully SEO-optimised, locally targeted long-form blog post targeting your keyword.</p>
      </div>
